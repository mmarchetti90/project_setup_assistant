...
# This script analyzes methylation coverage files from Bismark at the level of specific genes TSSs
...
parseArgs <- function() {
  ...
  # TSS bp extension upstream and downstream
  if("--tss_extension" %in% args) {
    
    tss_extension <- as.numeric(args[match("--tss_extension", args) + 1])
  ...
  return(c(path_to_files, samples_file, cpus, significance_threshold, significance_parameter, analysis_type, paired, dea_file, tss_extension, correct_batch))
  
}

### ---------------------------------------- ###

structureWorkDir <- function(samples_file) {
...
plotSamplesRelationship <- function(basedir, methylation_data) {
  ...
  write.table(pca_data$x, paste(basedir, "/RelationshipsPlot/pca_data.tsv", sep=""), col.names=NA, sep="\t", quote=FALSE)
...
extractDMC <- function(basedir, dmc_analysis, significance_threshold, significance_parameter) {
  ...
  # Saving to file
  write.table(dmc_regions, paste(basedir, "/DifferentialMethylation_Analysis/DMC_Regions.tsv", sep=""), row.names=F, sep="\t", quote=FALSE)
...
annotateDMC <- function(basedir, dmc_regions, tss) {
...
# Structuring workdir
basedir <- structureWorkDir(parameters[2])
...
dea <- read.delim(parameters[8], sep='\t', header=TRUE)
...
# Merging cytosine counts for TSS-level analysis
# N.B. For whatever reason, the function fails if run with save.db = TRUE
print("### Merging cytosine counts to TSS windows")
#methylation_data <- regionCounts(methylation_data, tss, save.db = TRUE, suffix = "tss_level")
methylation_data <- regionCounts(methylation_data, tss, save.db = FALSE)
...
if(0 %in% samples_info$Condition & 1 %in% samples_info$Condition) {
  ...
  if(parameters[6] == 'methylkit') {
    ...
    # UTest
    pvals <- apply(methylation_data[, 5 : ncol(methylation_data)], 1, function(m) { wilcox.test(m[x_cols], m[y_cols], alternative="two.sided", paired=as.logical(parameters[7]))$p.value })
  ...
  annotateDMC(basedir, dmc_regions, tss)
