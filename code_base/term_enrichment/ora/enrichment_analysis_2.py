...
def print_help():
    ...
    --gene_set : string path, required
        Path to gene set.
        One row per gene name.
...
def parse_args():
    ...
    # Load gene set
    gs_path = argv[argv.index('--gene_set') + 1]
    gs = [gene for gene in open(gs_path, 'r').read().split('\n') if gene != '']
    ...
    return gs, t2g, u, min_g, max_g, filt_g, out_p, e_type
    ...
    def plot_data(self, p_thr=0.05, max_categories=15):
        ...
        if data.shape[0] >= 3: # Only plot if there's at least 3 categories
            ...
            if len(set(data.behaviour)) == 2:
                ...
                figsize = (10, max(2.5, data.shape[0] // 2))
...
if '--help' in argv:
    ...
    gene_set, term2gene, universe, min_genes, max_genes, filter_genes, output_prefix, enrichment_type = parse_args()
