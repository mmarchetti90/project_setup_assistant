...
class zero_shot_filter:
    ...
    def forward(self, model: Callable, query: list[str], tags: list[str], score_threshold: float=0.75) -> dict[str]:
        ...
        for q in query:
            ...
            classification = model(q, candidate_labels=tags)

            q_tags_candidates = [(t, s) for t,s in zip(classification['labels'], classification['scores']) if s >= score_threshold]
...
if __name__ == '__main__':
    ...
    text_filter = zero_shot_filter()

    from src.models.zero_shot_model import init_zero_shot_model
    
    model = init_zero_shot_model(model_checkpoint='facebook/bart-large-mnli', device_map='auto')

    output = text_filter.forward(model=model, query=query, tags=tags, score_threshold=0.25)
